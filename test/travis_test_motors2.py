...
class MotorTest(unittest.TestCase):
	...
	self.assertEqual(ret.massage, "OFF", "motor off worong succeeded")
	with open("/dev/rtmotoren0","r") as f:
		data = f.readline()
		self.assertEqual(data,"0\n","wronf value in rtmotor0 at motor off")

	on = rospy.ServiceProxy('/motor_on',Trigger)
        ret = on()
        self.assertEqual(ret.success, True, "motor off does not succeeded")
        self.assertEqual(ret.massage, "ON", "motor on worong succeeded")
        with open("/dev/rtmotoren0","r") as f:
                data = f.readline()
                self.assertEqual(data,"1\n","wronf value in rtmotor0 at motor on")
